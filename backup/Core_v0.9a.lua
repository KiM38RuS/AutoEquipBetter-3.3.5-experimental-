...
function AEB:GetArrowFrame()
    ...
    arrow.text = arrow:CreateFontString(nil, "OVERLAY")
    arrow.text:SetFont("Fonts\\FRIZQT__.TTF", 20, "OUTLINE")
    arrow.text:SetText("|cff00ff00↑|r")
    arrow.text:SetPoint("CENTER")
    arrow.inUse = true
    ...
    for loc, data in pairs(bestBags) do
        ...
        if data.score > oldScore then
            ...
            if btn and btn:IsVisible() then
                ...
                arrow:SetParent(btn)
                arrow:SetPoint("BOTTOMRIGHT", btn, "BOTTOMRIGHT", -2, 2)
                arrow:Show()
            end
