        ...
        for slot = 1, GetContainerNumSlots(bag) do
            ...
            if link and IsEquippableItem(link) then
                ...
                if id and not blacklist[id] then
                    ...
                    -- Проверяем: 1. Есть ли у нас навык на этот подтип (если это броня или оружие)
                    local hasSkill = true
                    if itemType == "Доспехи" or itemType == "Оружие" then
                        hasSkill = self.knownSkills[subType]
                    end

                    -- 2. Подходит ли уровень
                    if hasSkill and (not minLvl or minLvl <= UnitLevel("player")) and loc then
...
-- Функция для обновления списка известных навыков
function AEB:UpdateKnownSkills()
    ...
    for i = 1, GetNumSkillLines() do
        ...
        if isHeader then
            ...
            -- Проверяем, находимся ли мы в нужных разделах
            if currentHeader == "Доспехи" or currentHeader == "Оружейные навыки" then
                -- В навыках доспехи часто пишутся как "Кожаные доспехи", 
                -- а в типе предмета просто "Кожаные". Отрезаем лишнее для совпадения.
